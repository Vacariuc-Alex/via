import React from "react"
import {cn, getTechLogos} from "@/commons/utils";
import Image from "next/image";
import {TechIconProps} from "@/commons/types";

const DisplayTechIcons = async ({technologies} : TechIconProps) => {
    const techIcons = await getTechLogos(technologies);
    return (
        <div className="tech-pills-container flex flex-row">
            {techIcons.slice(0, 5).map(({tech, url}, index) => (
                <div key={tech} className={cn("relative group bg-dark-300 rounded-full p-2 flex-center", index >= 1 && "-ml-3")}>
                    <span className="interview-tech-tooltip">{tech}</span>
                    <Image src={url} alt={tech} width={100} height={100} className="size-5"/>
                </div>
            ))}
        </div>
    )
}

export default DisplayTechIcons;
