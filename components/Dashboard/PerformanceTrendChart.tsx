...
const PerformanceTrendChart = async ({data}: PerformanceTrendChartProps) => {
    const t = await getTranslations("dashboard.chart");
    if (!data.length) {
        return (
            <div className="rounded-3xl border border-dashed border-cyan-400/20 bg-white/3 px-5 py-8 text-center">
                <p className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-300/75">{t("dailyAverageScore")}</p>
        ...
        <div className="rounded-3xl border border-cyan-400/20 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))] p-4 sm:p-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-light-100/75">
                        {t("averageInterviewScoreEachDay")}
                    </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.22em] text-light-100/65">{t("trackedDays")}</p>
            ...
            <div className="mt-6">
                <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-auto w-full" aria-labelledby="daily-average-score-chart-title">
                    <title id="daily-average-score-chart-title">{t("lineChartTitle")}</title>
                    <defs>
                        ...
                        return (
                            <g key={point.date}>
                                ...
                                <circle cx={point.x} cy={point.y} r="10" fill="transparent">
                                    <title>{`${formatTooltipDate(point.date)}\n${point.averageScore}/100 ${t("acrossAttempts", {attempts: point.attempts})}`}</title>
                                </circle>
                                {showAxisLabel ? (
